...
//****************************************************************************************************************************************************
	...
	for (int i = 0; i < matches.size(); i++)
	{
		...
		if ( resAngle <  thresholdSelectionAngle)
		{
			...
			max_disparity = std::max(max_disparity,(int)std::abs(keypoints_left[matches[i].queryIdx].pt.y - keypoints_rigth[matches[i].trainIdx].pt.y) + 1);
		}
	}
	cout << "Count filtered matches " << filteredMatches.size() << endl;
	cout << "Max disparity " << max_disparity << endl;
	clearDublicateMatches(filteredMatches);
	cout << "Count filtered matches after clean duplicate " << filteredMatches.size() << endl;
	...
	circle(img_left,keypoints_left[jointROIDMatch[0].queryIdx].pt,2,Scalar(40,100,255));
	circle(img_left,keypoints_left[jointROIDMatch[1].queryIdx].pt,2,Scalar(40,100,255));
	circle(img_left,keypoints_left[jointROIDMatch[2].queryIdx].pt,2,Scalar(40,100,255));
	circle(img_left,keypoints_left[jointROIDMatch[3].queryIdx].pt,2,Scalar(40,100,255));
	...
	Mat temp_right(5,max_disparity,img_right.type(), Scalar(0,0,0));
	Mat temp_left(3,3,img_left.type(), Scalar(0,0,0));
	Mat result_match(temp_right.rows - temp_left.rows + 1,temp_right.cols - temp_left.cols + 1,CV_32FC1);
	double minVal; double maxVal; Point minLoc; Point maxLoc;
	double err=0;

	Mat disp(left_point_end.y - left_point_start.y, left_point_end.x - left_point_start.x,img_left.type(),Scalar(0,0,0));
	Rect roi_left(0,0,3,3);
	Rect roi_right(0,0,5,max_disparity );
	double dispar_y  = 0;
	double dispar_x  = 0;
	double dispar = 0;
	int start=GetTickCount();
	//GaussianBlur( img_left, img_left, cv::Size( 3, 3 ), 0);
	//GaussianBlur( img_right, img_right, cv::Size( 3, 3 ), 0);
	
	for (int j = left_point_start.y + 3; j < left_point_end.y - 3; j++)
	{
		for (int i = left_point_start.x + 3; i < left_point_end.x - 3; i++)
		{
			...
			/*for (int k = 0; k < temp_right.rows; k++)
			{
				for (int l = 0; l < temp_right.cols; l++)
				{
					img_right.row(k + right_point_start.y + right_point_y_shift).col(l + right_point_start.x + right_point_x_shift).copyTo(temp_right.row(k).col(l));
				}
			}*/
			
			roi_right.x = right_point_start.x + right_point_x_shift;
			roi_right.y = right_point_start.y + right_point_y_shift;
			matchTemplate(img_right(roi_right),temp_left,result_match, CV_TM_SQDIFF);
			normalize( result_match, result_match, 0, 1, NORM_MINMAX, -1, Mat() );
			minMaxLoc( result_match, &minVal, &maxVal, &minLoc, &maxLoc, Mat() );
			
			dispar_y  = (right_point_y_shift - minLoc.y) - (j - left_point_start.y - 3);
			dispar_x  = (right_point_x_shift - minLoc.x) - (i - left_point_start.x - 3);
			dispar = sqrt(pow(dispar_x,2) + pow(dispar_y,2));
			disp.row(right_point_y_shift).col(right_point_x_shift) = Scalar(dispar * 10,1000/dispar,dispar);
			//circle(img_line, Point2f(i,j),1,Scalar(40,255,100));
			//circle(img_line, Point2f(img_left.cols,0) + Point2f(right_point_x_shift + right_point_start.x + minLoc.x,right_point_y_shift + right_point_start.y + minLoc.y),1,Scalar(40,255,100));
			//imshow("spisstav",img_line);
			//waitKey(250);
			//destroyWindow("spisstav");
			if(right_point_x_shift + right_point_start.x + temp_right.cols < right_point_end.x)
				right_point_x_shift++;
		}
		if(right_point_y_shift + right_point_start.y + temp_right.rows < right_point_end.y)
			right_point_y_shift++;
	...
	imshow("disp",disp);
	GaussianBlur( disp, disp, cv::Size( 5, 5 ), 0);
	imshow("GaussianBlur_disp",disp);
	waitKey(0);
	system("pause");
	//-- Конец
}
