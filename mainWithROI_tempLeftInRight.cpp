...
//****************************************************************************************************************************************************
	...
	for (int j = left_point_start.y + 2; j < left_point_end.y - 2; j++)
	{
		roi_right.y = right_point_start.y + right_point_y_shift;
		for (int i = left_point_start.x + 2; i < left_point_end.x - 2; i++)
		{
			img_left.row(j - 1).col(i - 1).copyTo(temp_left.row(1).col(1));
			img_left.row(j - 1).col(i).copyTo(temp_left.row(1).col(2));
			img_left.row(j - 1).col(i + 1).copyTo(temp_left.row(1).col(3));

			img_left.row(j).col(i - 1).copyTo(temp_left.row(2).col(1));
			img_left.row(j).col(i).copyTo(temp_left.row(2).col(2));
			img_left.row(j).col(i + 1).copyTo(temp_left.row(2).col(3));

			img_left.row(j + 1).col(i - 1).copyTo(temp_left.row(3).col(1));
			img_left.row(j + 1).col(i).copyTo(temp_left.row(3).col(2));
			img_left.row(j + 1).col(i + 1).copyTo(temp_left.row(3).col(3));

			roi_right.x = right_point_start.x + right_point_x_shift;
			Mat result_match_0(max_disparity + 1, temp_left.rows + 1, CV_32FC1);
			matchTemplate( img_right(roi_right),temp_left, result_match_0, CV_TM_SQDIFF_NORMED);
