...
//****************************************************************************************************************************************************
	...
	Mat temp_left(3,3,img_left.type(), Scalar(255,255,255));
	Mat temp_right(max_disparity,3,img_left.type(), Scalar(0,0,0));
	//Mat result_match(max_disparity + 1, temp_left.rows + 1, CV_32FC1);
	...
	for (int j = left_point_start.y + 1; j < left_point_end.y - 2; j++)
	{
		roi_right.y = right_point_start.y + right_point_y_shift;
		for (int i = left_point_start.x + 1; i < left_point_end.x - 2; i++)
		{
			...
			Mat result_match(max_disparity + 1, temp_left.rows + 1, CV_32FC1);
			matchTemplate( img_right(roi_right), temp_left, result_match, CV_TM_SQDIFF_NORMED);
			...
			disp.row(right_point_y_shift).col(right_point_x_shift) = Scalar(0,255/dispar,0);
	...
	imshow("disp",disp);
	GaussianBlur( disp, disp, cv::Size( 3, 3 ), 0);
