...
void ImgPlusCanny(const Mat& src, Mat& dst);
...
int main( int argc, char** argv )
{
	...
	brightnessNormalizationEqualizeHist(img_left,img_left,1);
	brightnessNormalizationEqualizeHist(img_right,img_right,1);
	...
	cout << "Кiлькiсть пар особливих точок = " << matches.size() << endl;
	...
	cout << "Кiлькiсть прямих лiнiй на лiвому зображенi = " << leftLines.size() << endl;

	vector<Vec4i> rightLines = findLines(img_right, rho, theta, threshold, minLineLength, maxLineGap);
	cout << "Кiлькiсть прямих лiнiй на правому зображенi = " << rightLines.size() << endl;
	...
	cout << "Кiлькiсть прямих лiнiй на лiвому зображенi, що мають >= двох особливих точок = " << leftLineDot.size() << endl;
	...
	cout << "Кiлькiсть прямих лiнiй на правому зображенi, що мають >= двох особливих точок = " << rightLineDot.size() << endl;
	
	vector<Vec9f> result = findSimilarLinesAndAngleBetweenThem(leftLineDot,rightLineDot);
	cout << "Кiлькiсть вiдповiдних прямих лiнiй = " << result.size() << endl;

	for (int i = 0; i < result.size(); i++)
	{
		cout << "Кут повороту зображень = " << result[i][8] << endl;
	}

	rotation(result[0][8],img_right);

	for (int i = 0; i < leftLineDot.size(); i++)
	{
		line(img_line,Point2i(leftLineDot[i].line[0],leftLineDot[i].line[1]),Point2i(leftLineDot[i].line[2],leftLineDot[i].line[3]),Scalar(255,255,0),2);
	}
	for (int i = 0; i < rightLineDot.size(); i++)
	{
		line(img_line,Point2i(rightLineDot[i].line[0],rightLineDot[i].line[1]) + Point2i(img_left.cols,0),Point2i(rightLineDot[i].line[2],rightLineDot[i].line[3])  + Point2i(img_left.cols,0),Scalar(255,255,0),2);
	}
	for (int i = 0; i < result.size(); i++)
	{
		line(img_line,Point2i(result[i][0],result[i][1]),Point2i(result[i][2],result[i][3]),Scalar(40,200,40),2);
		line(img_line,Point2i(result[i][4],result[i][5]) + Point2i(img_left.cols,0),Point2i(result[i][6],result[i][7]) + Point2i(img_left.cols,0),Scalar(40,200,40),2);
	}
...
//****************************************************************************************************************************************************
	...
	cout << "Кiлькiсть пар особливих точок пiсля видалення дублiкатiв = " << matches.size() << endl;
	...
	cout << "Кiлькiсть пар особливих точок пiсля фiльтрування = " << filteredMatches.size() << endl;
	...
	cout << "Максимальне змiщення вiдповiдних точок = " << max_disparity << endl;
	cout << "Мiнiмальне змiщення вiдповiдних точок = " << min_disparity << endl;
	...
	line(img_line, leftROI[0],leftROI[1],Scalar(120,255,255),2);
	line(img_line, leftROI[1],leftROI[2],Scalar(120,255,255),2);
	line(img_line, leftROI[2],leftROI[3],Scalar(120,255,255),2);
	line(img_line, leftROI[0],leftROI[3],Scalar(120,255,255),2);
	...
	line(img_line, Point(img_left.cols,0) + rightROI[0], Point(img_left.cols,0) + rightROI[1],Scalar(120,255,255),2);
	line(img_line, Point(img_left.cols,0) + rightROI[1], Point(img_left.cols,0) + rightROI[2],Scalar(120,255,255),2);
	line(img_line, Point(img_left.cols,0) + rightROI[2], Point(img_left.cols,0) + rightROI[3],Scalar(120,255,255),2);
	line(img_line, Point(img_left.cols,0) + rightROI[0], Point(img_left.cols,0) + rightROI[3],Scalar(120,255,255),2);
	...
	Rect roi_left( 0, 0, max_disparity + min_disparity, 5);
	Rect roi_right( 0, 0, 5, 5);
	...
	for (int j = right_point_start.y; j < right_point_end.y; j++)
	{
		roi_left.y = left_point_start.y + left_point_y_shift;
		roi_right.y = right_point_start.y + right_point_y_shift;

		for (int i = right_point_start.x; i < right_point_end.x; i++)
		{
			roi_right.x = right_point_start.x + right_point_x_shift;
			if (left_point_start.x + left_point_x_shift - min_disparity >= 0)
				roi_left.x = left_point_start.x + left_point_x_shift - min_disparity;
			else
				roi_left.x = left_point_start.x + left_point_x_shift;

			Mat result_match(max_disparity + 1, roi_right.height + 1, CV_32FC1);
			matchTemplate( img_left(roi_left),img_right(roi_right), result_match, CV_TM_SQDIFF_NORMED);
			normalize( result_match, result_match, 0, 1, NORM_MINMAX, -1, Mat() );
			minMaxLoc( result_match, &minVal, &maxVal, &minLoc, &maxLoc, Mat() );
			result_match.release();

			dispar_y  = (roi_left.y + minLoc.y - left_point_start.y) - (roi_right.y - right_point_start.y);
			dispar_x  = (roi_left.x + minLoc.x - left_point_start.x) - (roi_right.x - right_point_start.x);
			dispar = sqrt(pow(dispar_x,2) + pow(dispar_y,2));
			
			disp.row(j - right_point_start.y).col(i - right_point_start.x) = dispar;
		...
		right_point_x_shift = 0;
	}
	left_point_y_shift = 0;
	right_point_y_shift = 0;
	int end=GetTickCount();
	std::cout <<"Час стереоронструкцiї зображень = " << (end-start)/1000.0 << std::endl;
...
void ImgPlusCanny(const Mat& src, Mat& dst_out)
{
	Mat src_gray;
