...
//****************************************************************************************************************************************************
	...
	Rect roi_left( 0, 0, max_disparity + min_disparity, temp_right.rows);
	Rect roi_right( 0, 0, max_disparity + min_disparity, temp_right.rows);
	GaussianBlur(img_left,img_left,Size(5,5),0,0);
	GaussianBlur(img_right,img_right,Size(5,5),0,0);
	...
	for (int j = right_point_start.y + 2; j < right_point_end.y - 2; j++)
	{
		...
		for (int i = right_point_start.x + 2; i < right_point_end.x - 2; i++)
		{
			...
			Mat result_match(max_disparity + 1, temp_right.rows + 1, CV_32FC1);
			matchTemplate( img_left(roi_left),temp_right, result_match, CV_TM_SQDIFF);
			...
			dispar_y  = (roi_left.y + minLoc.y - left_point_start.y + 2) - (j - right_point_start.y);
			dispar_x  = (roi_left.x + minLoc.x - left_point_start.x + 2) - (i - right_point_start.x);
