...
int main( int argc, char** argv )
{
	...
	Mat kernel_matrix = Mat(3,3,CV_32F,kernel);

	filter2D(img_left, img_left, -1 , kernel_matrix, Point( -1, -1 ), 0, BORDER_DEFAULT );
	filter2D(img_right, img_right, -1 , kernel_matrix, Point( -1, -1 ), 0, BORDER_DEFAULT );
	*/
...
//****************************************************************************************************************************************************
	...
	Rect roi_right(0,0,max_disparity, 3);
	Rect roi_left( 0, 0, 2, 3);
	...
	Mat temp_left_0(3,3,img_left.type(), Scalar(0,0,0));
	Mat temp_left_255(3,3,img_left.type(), Scalar(255,255,255));
	
	int start=GetTickCount();
	//GaussianBlur( img_left, img_left, Size( 3, 3 ), 0, 0 );
	//GaussianBlur( img_right, img_right, Size( 3, 3 ), 0, 0 );



	for (int j = left_point_start.y; j < left_point_end.y - 2; j++)
	{
		roi_right.y = right_point_start.y + right_point_y_shift;
		for (int i = left_point_start.x; i < left_point_end.x - 2; i++)
		{
			img_left.row(j - 1).col(i - 1).copyTo(temp_left_0.row(0).col(0));
			//img_left.row(j - 1).col(i).copyTo(temp_left_0.row(0).col(1));
			//img_left.row(j - 1).col(i + 1).copyTo(temp_left_0.row(0).col(2));

			img_left.row(j).col(i - 1).copyTo(temp_left_0.row(1).col(0));
			img_left.row(j).col(i).copyTo(temp_left_0.row(1).col(1));
			img_left.row(j).col(i + 1).copyTo(temp_left_0.row(1).col(2));

			img_left.row(j + 1).col(i - 1).copyTo(temp_left_0.row(2).col(0));
			//img_left.row(j + 1).col(i).copyTo(temp_left_0.row(2).col(1));
			//img_left.row(j + 1).col(i + 1).copyTo(temp_left_0.row(2).col(2));

			

			roi_right.x = right_point_start.x + right_point_x_shift;
			Mat result_match_0(max_disparity + 1, temp_left_0.rows + 1, CV_32FC1);
			matchTemplate( img_right(roi_right),temp_left_0, result_match_0, CV_TM_SQDIFF_NORMED);
			...
			dispar_y  = (right_point_y_shift + minLoc.y) - (j - left_point_start.y);
			dispar_x  = (right_point_x_shift + minLoc.x) - (i - left_point_start.x);
			dispar_0 = sqrt(pow(dispar_x,2) + pow(dispar_y,2));

			/*if (dispar_0 >= min_disparity)
				disp.row(right_point_y_shift).col(right_point_x_shift) = dispar_0;
			else 
				disp.row(right_point_y_shift).col(right_point_x_shift) = 0;
				*/

			img_left.row(j - 1).col(i - 1).copyTo(temp_left_255.row(0).col(0));
			//img_left.row(j - 1).col(i).copyTo(temp_left_0.row(0).col(1));
			//img_left.row(j - 1).col(i + 1).copyTo(temp_left_0.row(0).col(2));

			img_left.row(j).col(i - 1).copyTo(temp_left_255.row(1).col(0));
			img_left.row(j).col(i).copyTo(temp_left_255.row(1).col(1));
			img_left.row(j).col(i + 1).copyTo(temp_left_255.row(1).col(2));

			img_left.row(j + 1).col(i - 1).copyTo(temp_left_255.row(2).col(0));
			//img_left.row(j + 1).col(i).copyTo(temp_left_0.row(2).col(1));
			//img_left.row(j + 1).col(i + 1).copyTo(temp_left_0.row(2).col(2));

			Mat result_match_255(max_disparity + 1, temp_left_255.rows + 1, CV_32FC1);
			matchTemplate( img_right(roi_right),temp_left_255, result_match_255, CV_TM_SQDIFF_NORMED);
			normalize( result_match_255, result_match_255, 0, 1, NORM_MINMAX, -1, Mat() );
			minMaxLoc( result_match_255, &minVal, &maxVal, &minLoc, &maxLoc, Mat() );
			result_match_255.release();

			dispar_y  = (right_point_y_shift + minLoc.y) - (j - left_point_start.y);
			dispar_x  = (right_point_x_shift + minLoc.x) - (i - left_point_start.x);
			dispar_255 = sqrt(pow(dispar_x,2) + pow(dispar_y,2));
			
			if (max(dispar_0,dispar_255) >= min_disparity)
				disp.row(right_point_y_shift).col(right_point_x_shift) = max(dispar_0,dispar_255);

			if(right_point_x_shift + right_point_start.x + max_disparity <= right_point_end.x)
				right_point_x_shift++;
		}
		if(right_point_y_shift + right_point_start.y + roi_right.height <= right_point_end.y)
			right_point_y_shift++;
